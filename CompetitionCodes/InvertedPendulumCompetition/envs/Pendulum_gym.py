...
class MiniArmPendulumEnv(gym.Env):
    ...
    def __init__(self, render_mode: Optional[str] = None):
        ...
        for i in range(self.model.ngeom):
            if self.model.geom_bodyid[i] == self.ee_body_id:
                ee_geom_ids.append(i)
        self.ee_geom_ids = np.array(ee_geom_ids, dtype=np.int32)
    ...
    def _check_ground_contact(self) -> bool:
        ...
        for i in range(contract_id):
            ...
            if g1 == self.ee_grond_id and g2 in self.ee_geom_ids:
                return True
